...
(function (global) {
  ...
  function openDetail(item) {
    ...
    // Part 3 3.4: dialog semantics + Escape-to-close on the overlay
    ov.setAttribute("role", "dialog"); ov.setAttribute("aria-modal", "true"); ov.setAttribute("aria-label", item.title || "Item details");
    function onKey(e) { if (e.key === "Escape") close(); }
    doc.addEventListener("keydown", onKey);
    function close() { doc.removeEventListener("keydown", onKey); ov.classList.remove("on"); setTimeout(function () { if (ov.parentNode) ov.parentNode.removeChild(ov); }, 220); }
